...
def login():
	...
	if access_token is None:
		...
		print(f"Xin chào, {login_response.get('name')}!\n")
		global refresh_token
		global token_expiry_time
		refresh_token = login_response.get("refresh_token")
		token_expiry_str = login_response.get(".expires")
		token_expiry_time = datetime.strptime(token_expiry_str, "%a, %d %b %Y %H:%M:%S GMT")
		return access_token
	
def expiry_check():
	global token_expiry_time
	current_time = datetime.utcnow()
	if token_expiry_time - current_time <= timedelta(minutes=15):
		refresh_response = requests.post("https://ftugate.ftu.edu.vn/api/auth/login",
			headers={"Authorization": f"Bearer {access_token}", "Content-Type": "application/x-www-form-urlencoded"},
			data={"grant_type": "refresh_token", "refresh_token": refresh_token}).json()
		
		access_token = refresh_response.get("access_token")
		refresh_token = refresh_response.get("refresh_token")
		token_expiry_str = refresh_response.get(".expires")
		
		if access_token:
			token_expiry_time = datetime.strptime(token_expiry_str, "%a, %d %b %Y %H:%M:%S GMT")
			print("Đã làm mới phiên đăng nhập")
			return False
		else:
			print("Không thể làm mới phiên đăng nhập. Vui lòng đăng nhập lại.")
			access_token=None
			return True
	else:
		return False
	...
	if not expiry_check():
		...
		if reg_response.status_code==200:
			...
			if data["is_thanh_cong"]==True:
				...
				print(data["thong_bao_loi"])
				if data["thong_bao_loi"].startswith("Vi phạm học trước"):
					return True
				else:
					return False
		else:
...
def format_semester_name(hoc_ky):
	hoc_ky = str(hoc_ky)
	year = hoc_ky[:4]  # First 4 digits are the year part (e.g., 2024)
	semester_code = hoc_ky[-1]  # Last digit determines the semester (1, 2, or 3)
	
	if semester_code == "1":
		semester_name = f"Học kỳ 1 Năm học {year}-{int(year) + 1}"
	elif semester_code == "2":
		semester_name = f"Học kỳ 2 Năm học {year}-{int(year) + 1}"
	elif semester_code == "3":
		semester_name = f"Học kỳ Hè Năm học {year}-{int(year) + 1}"
	else:
		semester_name = "Học kỳ N/A Năm học N/A"
	...
	while True:
		...
		if first_run:
			...
			class_list = (requests.post(
				...
				headers={"Authorization": f"Bearer {access_token}", "Content-Type": "application/json"},
				json={"is_CVHT": False, "additional": {"paging": {"limit": 99999,"page": 1},"ordering": [{"name": "","order_type": ""}]}}
			)).json()
			first_run = False
